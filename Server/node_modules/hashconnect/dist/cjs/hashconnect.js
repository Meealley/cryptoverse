...
exports.HashConnect = void 0;
const ts_typed_events_1 = require("ts-typed-events");
const relay_1 = require("./types/relay");
const message_1 = require("./message");
const hashconnect_1 = require("./types/hashconnect");
const provider_1 = require("./provider/provider");
const signer_1 = require("./provider/signer");
...
class HashConnect {
    constructor(debug) {
        ...
        this.debug = false;
        this.status = hashconnect_1.HashConnectConnectionState.Disconnected; //do we even need this?
        this.hcData = {
            topic: "",
            pairingString: "",
            encryptionKey: "",
        ...
        this.foundIframeEvent = new ts_typed_events_1.Event();
        ...
        this.connectionStatusChangeEvent = new ts_typed_events_1.Event();
        this.authRequestEvent = new ts_typed_events_1.Event();
        this.signRequestEvent = new ts_typed_events_1.Event();
    ...
    async init(metadata, network, singleAccount = true) {
        return new Promise(async (resolve) => {
            ...
            if (!this.loadLocalData()) {
                ...
                this.status = hashconnect_1.HashConnectConnectionState.Connected;
                this.connectionStatusChangeEvent.emit(hashconnect_1.HashConnectConnectionState.Connected);
            }
            else {
                if (this.debug)
                    console.log("hashconnect - Found saved local data", this.hcData);
                this.metadata.encryptionKey = this.hcData.encryptionKey;
                this.status = hashconnect_1.HashConnectConnectionState.Connecting;
                this.connectionStatusChangeEvent.emit(hashconnect_1.HashConnectConnectionState.Connecting);
                initData.pairingString = this.hcData.pairingString;
                ...
                if (this.hcData.pairingData.length > 0) {
                    this.status = hashconnect_1.HashConnectConnectionState.Paired;
                    this.connectionStatusChangeEvent.emit(hashconnect_1.HashConnectConnectionState.Paired);
                }
            }
            if (this.debug)
    ...
    async clearConnectionsAndData() {
        ...
        this.status = hashconnect_1.HashConnectConnectionState.Disconnected;
        this.connectionStatusChangeEvent.emit(hashconnect_1.HashConnectConnectionState.Disconnected);
    }
    /**
     * Send functions
     */
    async sendTransaction(topic, transaction) {
        ...
        const msg = await this.messages.prepareSimpleMessage(message_1.RelayMessageType.Transaction, transaction, topic, this);
    ...
    async requestAdditionalAccounts(topic, message) {
        const msg = await this.messages.prepareSimpleMessage(message_1.RelayMessageType.AdditionalAccountRequest, message, topic, this);
        await this.relay.publish(topic, msg, this.encryptionKeys[topic]);
    ...
    async sendAdditionalAccounts(topic, message) {
        ...
        const msg = await this.messages.prepareSimpleMessage(message_1.RelayMessageType.AdditionalAccountResponse, message, topic, this);
        await this.relay.publish(topic, msg, this.encryptionKeys[topic]);
        return message.id;
    }
    async sendTransactionResponse(topic, message) {
        ...
        const msg = await this.messages.prepareSimpleMessage(message_1.RelayMessageType.TransactionResponse, message, topic, this);
        await this.relay.publish(topic, msg, this.encryptionKeys[topic]);
        return message.id;
    }
    async pair(pairingData, accounts, network) {
        ...
        const payload = await this.messages.prepareSimpleMessage(message_1.RelayMessageType.ApprovePairing, msg, msg.topic, this);
    ...
    async reject(topic, reason, msg_id) {
        ...
        const msg = await this.messages.prepareSimpleMessage(message_1.RelayMessageType.RejectPairing, reject, topic, this);
    ...
    async acknowledge(topic, pubKey, msg_id) {
        ...
        const ackPayload = await this.messages.prepareSimpleMessage(message_1.RelayMessageType.Acknowledge, ack, topic, this);
    ...
    async authenticate(topic, account_id, server_signing_account, serverSignature, payload) {
        ...
        const msg = await this.messages.prepareSimpleMessage(message_1.RelayMessageType.AuthenticationRequest, message, topic, this);
        await this.relay.publish(topic, msg, this.encryptionKeys[topic]);
        this.sendEncryptedLocalTransaction(msg);
    ...
    async sendAuthenticationResponse(topic, message) {
        ...
        const msg = await this.messages.prepareSimpleMessage(message_1.RelayMessageType.AuthenticationResponse, message, topic, this);
        await this.relay.publish(topic, msg, this.encryptionKeys[topic]);
        return message.id;
    }
    /**
    ...
    async sign(topic, account_id, payload) {
        ...
        const msg = await this.messages.prepareSimpleMessage(message_1.RelayMessageType.SigningRequest, message, topic, this);
        await this.relay.publish(topic, msg, this.encryptionKeys[topic]);
        this.sendEncryptedLocalTransaction(msg);
    ...
    async sendSigningResponse(topic, message) {
        if (message.userSignature)
            message.userSignature = Buffer.from(message.userSignature).toString("base64");
        const msg = await this.messages.prepareSimpleMessage(message_1.RelayMessageType.SigningResponse, message, topic, this);
        await this.relay.publish(topic, msg, this.encryptionKeys[topic]);
        return message.id;
    }
    /**
    ...
    getProvider(network, topicId, accountToSign) {
        return new provider_1.HashConnectProvider(network, this, topicId, accountToSign);
    }
    getSigner(provider) {
        return new signer_1.HashConnectSigner(this, provider, provider.accountToSign, provider.topicId);
    }
...
exports.HashConnect = HashConnect;
